...
export default function DemonArkCheckoutPage() {
  ...
  const handleCheckout=useCallback(async()=>{
    ...
    try{
      ...
      const checkoutUser:CheckoutUser={};requiredIdentifiers.forEach(id=>{const v=identifierValues[id]?.trim();if(v)(checkoutUser as Record<string,string>)[id]=v;});const origin=location.origin;const body:CheckoutBody={products,redirect_success_checkout:`${origin}/checkout/success`,redirect_canceled_checkout:`${origin}/checkout/canceled`};if(Object.keys(checkoutUser).length)body.user=checkoutUser;const result=await createCheckout(store.id,body);setIsRedirecting(true);location.href=result.url;
  ...
  return <div className="pt-24 lg:pt-28 pb-16 animate-fade-in"><div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
