...
export default function DemonArkCheckoutPage() {
  const navigate=useNavigate(); const {items,removeItem,updateQuantity}=useCart(); const {store}=useStore(); const {addToast}=useToast(); const {token,user,connect}=useTip4ServAuth(); const currency=store?.currency;
  ...
  const [vipStatus,setVipStatus]=useState<VipStatus|null>(null); const [vipLoading,setVipLoading]=useState(false); const [vipCoupon,setVipCoupon]=useState<VipCoupon|null>(null); const [pendingPaymentUrl,setPendingPaymentUrl]=useState<string|null>(null);
  ...
  useEffect(()=>{
    ...
    fetch(`${apiBaseUrl}/api/account/vip-status`,{headers:{Authorization:`Bearer ${token}`,Accept:'application/json'}})
      ...
      .then(data=>{if(!cancelled)setVipStatus({active:Boolean(data?.active),discount_percent:Number(data?.discount_percent||0)});})
  ...
  },[acceptedTerms,store,requiredIdentifiers,identifierValues,discordTag,selectedServer,items,addToast,token,apiBaseUrl,vipStatus]);
