...
/* Keep critical homepage artwork strongly referenced for the lifetime of the app.
   iOS/WebKit can evict off-screen rasterized images aggressively; retaining the
   Image objects + decoding them up front makes returning to the logo/cards much
   less likely to show a blank frame while Safari re-rasterizes the asset. */
const retainedHomeImages = new Map<string, HTMLImageElement>();

function retainHomeImage(src?: string | null) {
  if (!src || typeof Image === 'undefined' || retainedHomeImages.has(src)) return;
  ...
  image.src = src;
  retainedHomeImages.set(src, image);
...
export default function Hero() {
  ...
  useEffect(() => {
    const criticalImages = [
      '/demonarkbackg.png',
      store?.logo,
      '/Demonarkdiscordlgo.png',
      '/vipcoinpile.png?v=20260911',
      '/VIPCOINLOGO.png?v=20260911',
      '/demonarkvipbanner.png?v=20260911',
      '/misccatagorylogo.png?v=20260911',
      '/privateservercatagorylogo.png?v=20260911',
      vipCoinProductImage,
      demonVipProductImage,
      ...featuredProducts.map((product) => product.image),
    ];

    criticalImages.forEach(retainHomeImage);
  }, [store?.logo, vipCoinProductImage, demonVipProductImage, featuredProducts]);
      ...
      <div className="relative z-10 mx-auto flex max-w-7xl flex-col items-center px-4 pb-16 pt-24 text-center sm:px-6 sm:pt-24 lg:px-8 lg:pt-28">
        {store?.logo && (
          <Link to="/" className="da-logo-pulse group block" aria-label="Back to DemonArk home">
            <img src={store.logo} alt={title} loading="eager" fetchPriority="high" decoding="sync" className="da-logo-base h-32 w-auto object-contain drop-shadow-[0_10px_35px_rgba(0,0,0,.95)] transition-transform duration-150 ease-out group-hover:scale-[.90] sm:h-40 lg:h-48" />
          </Link>
        )}
        ...
        <div className="mt-8 flex w-full max-w-sm flex-col items-stretch gap-4 sm:w-auto sm:min-w-[360px]">
          ...
          <a href={discordUrl} target="_blank" rel="noopener noreferrer" className="da-action-pulse da-discord-button group inline-flex items-center justify-center gap-3 rounded-xl border border-white/12 bg-[#18181b]/90 px-8 py-3.5 text-sm font-black uppercase tracking-[.08em] text-white shadow-[0_12px_35px_rgba(0,0,0,.35)] transition duration-300 hover:-translate-y-1 hover:scale-[1.03] hover:border-red-400/45 hover:bg-[#202024]">
            <img src="/Demonarkdiscordlgo.png" alt="" loading="eager" fetchPriority="high" decoding="sync" className="h-8 w-8 object-contain transition-transform duration-200 group-hover:rotate-[-5deg] group-hover:scale-110" /> Join Discord <ArrowRight className="h-5 w-5 transition-transform duration-300 group-hover:translate-x-1.5" />
          </a>
        </div>
          ...
          <div className="min-h-[420px] lg:min-h-[560px]">
            {currentFeature ? (
              <Link key={currentFeature.id} to={`/product/${currentFeature.slug}`} className="da-feature-slide group da-panel relative flex h-full min-h-[420px] overflow-hidden rounded-3xl lg:min-h-[560px]" style={lightweightMobile ? { backdropFilter: 'none', WebkitBackdropFilter: 'none', background: 'rgba(17,17,19,.94)' } : undefined}>
                {currentFeature.image ? <img src={currentFeature.image} alt={currentFeature.name} loading="eager" fetchPriority="high" decoding="sync" className="absolute inset-0 h-full w-full object-cover transition duration-1000 group-hover:scale-[1.06]" /> : <div className="absolute inset-0 bg-gradient-to-br from-red-950 via-zinc-900 to-black" />}
...
function PortalCard({ image, fallbackImage, title, subtitle, href }: { image: string; fallbackImage?: string; title: string; subtitle: string; href: string }) {
  ...
  return (
    <Link to={href} className="da-portal group relative h-[175px] overflow-hidden rounded-2xl border border-white/10 bg-[#131315]/85 transition duration-500 hover:-translate-y-1.5 hover:border-red-400/60 hover:shadow-[0_24px_60px_rgba(127,29,29,.30)] sm:h-[230px] lg:h-full lg:min-h-[270px]">
      <img src={src} alt={title} loading="eager" fetchPriority="high" decoding="sync" onError={() => { if (fallbackImage && src !== fallbackImage) setSrc(fallbackImage); }} className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-110 group-hover:saturate-[1.14]" />
