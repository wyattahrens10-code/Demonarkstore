    ...
    <section className="da-home">
      ...
      <div className="relative z-10 mx-auto flex max-w-7xl flex-col items-center px-4 pb-16 pt-24 text-center sm:px-6 sm:pt-24 lg:px-8 lg:pt-28">
        {store?.logo && (
          <Link to="/" className="da-logo-pulse group block" aria-label="Back to DemonArk home">
            <img
              src={store.logo}
              alt={title}
              className="h-32 w-auto object-contain drop-shadow-[0_10px_35px_rgba(0,0,0,.95)] transition-transform duration-150 ease-out group-hover:scale-[.90] sm:h-40 lg:h-48"
            />
          </Link>
        )}

        <p className="mt-9 max-w-md text-sm font-medium leading-relaxed text-white/80 drop-shadow-[0_2px_10px_#000] sm:text-base">
        ...
        <div className="mt-8 flex w-full max-w-sm flex-col items-stretch gap-4 sm:w-auto sm:min-w-[360px]">
          ...
          <a
            href={discordUrl || '#'}
            onClick={(event) => { if (!discordUrl) event.preventDefault(); }}
            target={discordUrl ? '_blank' : undefined}
            rel={discordUrl ? 'noopener noreferrer' : undefined}
            className="da-action-pulse da-discord-button group inline-flex items-center justify-center gap-3 rounded-xl border border-white/12 bg-[#18181b]/90 px-8 py-3.5 text-sm font-black uppercase tracking-[.08em] text-white shadow-[0_12px_35px_rgba(0,0,0,.35)] transition duration-300 hover:-translate-y-1 hover:scale-[1.03] hover:border-red-400/45 hover:bg-[#202024]"
          >
            <img src="/Demonarkdiscordlgo.png" alt="" className="h-8 w-8 object-contain transition-transform duration-200 group-hover:rotate-[-5deg] group-hover:scale-110" />
            Join Discord
            <ArrowRight className="h-5 w-5 transition-transform duration-300 group-hover:translate-x-1.5" />
          </a>
        </div>

        <div className="mt-6 inline-flex items-center gap-2 rounded-full border border-white/10 bg-[#151517]/78 px-4 py-2 text-[11px] font-semibold uppercase tracking-[.12em] text-zinc-300 backdrop-blur-md">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 pb-40 pt-8 sm:px-6 lg:px-8">
        <div className="mb-8 flex items-end justify-between gap-4">
          ...
          {featuredProducts.length > 1 && (
            <div className="flex gap-2">
              <button onClick={previousFeature} aria-label="Previous featured item" className="flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-[#171719]/85 text-zinc-200 transition hover:-translate-y-0.5 hover:border-red-500/50 hover:bg-[#202023] hover:text-white"><ArrowLeft className="h-5 w-5" /></button>
              <button onClick={nextFeature} aria-label="Next featured item" className="flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-[#171719]/85 text-zinc-200 transition hover:-translate-y-0.5 hover:border-red-500/50 hover:bg-[#202023] hover:text-white"><ArrowRight className="h-5 w-5" /></button>
            </div>
          )}
        </div>
        ...
        {featuredProducts.length > 1 && (
          <div className="mt-6 flex justify-center gap-2">
            {featuredProducts.map((product, index) => <button key={product.id} onClick={() => setActiveFeature(index)} aria-label={`Show ${product.name}`} className={`h-2.5 rounded-full transition-all ${index === activeFeature ? 'w-9 bg-red-500' : 'w-2.5 bg-white/25 hover:bg-white/45'}`} />)}
          </div>
        )}
      </div>
  ...
  return (
    <Link to={href} className="da-portal group relative h-[280px] overflow-hidden rounded-2xl border border-white/10 bg-[#131315]/85 transition duration-500 hover:-translate-y-2 hover:border-red-400/60 hover:shadow-[0_28px_80px_rgba(127,29,29,.34)] sm:h-[340px]">
      ...
      <div className="absolute inset-x-0 bottom-0 flex items-end justify-between p-6">
        <div><div className="text-2xl font-black text-white drop-shadow-lg">{title}</div><div className="mt-1 text-xs font-bold uppercase tracking-[.18em] text-red-300">{subtitle}</div></div>
        <div className="flex h-11 w-11 items-center justify-center rounded-full border border-red-400/40 bg-red-700/90 text-white shadow-[0_0_25px_rgba(239,68,68,.28)] transition duration-300 group-hover:scale-115 group-hover:bg-red-500 group-hover:shadow-[0_0_34px_rgba(239,68,68,.55)]"><ArrowRight className="h-5 w-5 transition-transform duration-300 group-hover:translate-x-1" /></div>
      </div>
