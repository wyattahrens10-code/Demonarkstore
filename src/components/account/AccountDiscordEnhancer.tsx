import { useLocation } from 'react-router-dom';
import { useTip4ServAuth } from '../../lib/tip4servAuth';
import DiscordLinkCard from './DiscordLinkCard';
import AccountPlayerIdentityCard from './AccountPlayerIdentityCard';

export default function AccountDiscordEnhancer() {
  const location = useLocation();
  const { token, ready, loading } = useTip4ServAuth();

  if (location.pathname !== '/account' || !ready || loading || !token) return null;

  return (
    <section className="mx-auto -mt-6 max-w-6xl px-4 pb-16 sm:px-6 lg:px-8">
      <div className="mb-4">
        <div className="text-[11px] font-black uppercase tracking-[.2em] text-red-400">DemonArk identity</div>
        <h2 className="mt-1 text-xl font-black text-white">Connected accounts</h2>
        <p className="mt-1 text-sm text-zinc-500">Keep your ARK identity and linked services synced to your DemonArk account for faster checkout and future automatic delivery.</p>
      </div>
      <div className="max-w-2xl space-y-4">
        <AccountPlayerIdentityCard />
        <DiscordLinkCard />
      </div>
    </section>
  );
}
